...
function getToolName(tool: AgentTool<any>): string {
	return (tool as { name?: string }).name ?? tool.label;
}
		...
		for (const chunk of this.message.content) {
			if (chunk.type === "text" && chunk.text.trim() !== "") {
				...
				if (!this.hideToolCalls) {
					const tool = this.tools?.find((t) => getToolName(t) === chunk.name);
...
export class ToolMessage extends LitElement {
	...
	override render() {
		const toolName = this.tool ? getToolName(this.tool) : this.toolCall.name;
