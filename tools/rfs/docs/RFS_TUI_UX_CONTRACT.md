...
The TUI uses a Pi-inspired visual style for hierarchy and spacing, but it is still RFS and not Pi. Color is decorative; `NO_COLOR` should suppress ANSI output when supported. On interactive entry, the screen is cleared once before the header is rendered; redirected runs must not clear. Final assistant responses use a Markdown-lite renderer for readability: headings, bullets, simple bold/inline code cleanup, basic `a^2 -> a²` normalization, a small LaTeX-lite pass for common Greek letters, subscripts, `\text{...}`, and `\boxed{...}` cleanup, and compact text diagrams are preserved verbatim when they materially improve the explanation. Mermaid is not rendered in this phase.
